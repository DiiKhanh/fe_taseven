...
const ModalLocation = (props: Props) => {
  ...
  useEffect(() => {
    GeoLocation.getCurrentPosition(
      ...
      error => {
        console.log('error load cur pos' ,error);
      },
      {},
    );
  }, []);
  ...
  useEffect(() => {
    GeoCoder.from(addressSelected)
      ...
      .catch(error => console.log('error map choice', error));
  }, [addressSelected]);
  ...
  const handleGetAddressFromPosition = ({
    ...
    GeoCoder.from(latitude, longitude)
      .then(data => {
        console.log('data select pos', data);
        console.log('data real', data.results[0].address_components[0]);
      })
      .catch(error => {
        console.log('error gg maps', error);
      });
  };
  ...
  return (
    <Modal animationType="slide" visible={visible} style={{flex: 1}}>
      <View style={{paddingVertical: 10}}>
        <RowComponent
          justify="flex-end"
          styles={{marginVertical: 0, paddingHorizontal: 20}}>
          <View style={{flex: 1}}>
            <InputComponent
